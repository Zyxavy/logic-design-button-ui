...
// State
bool  powerOn    = true;
int   brightness = 75;   // 0-100 (kept for legacy BRIGHTNESS: command)

// Custom solid color (set via COLOR:R,G,B — already brightness-scaled by browser)
...
void handleSerial() {
  ...
  // COLOR:R,G,B — exact scaled values sent by browser
  if (line.startsWith("COLOR:")) {
    ...
    if (c1 > 0 && c2 > 0) {
      ...
      currentMode = SOLID;   // jump to solid when color is picked
    }
    return;
  }
...
void runMode() {
  ...
  switch (currentMode) {
    ...
    case SOLID:
      // Use color set by COLOR: command (pre-scaled by browser)
